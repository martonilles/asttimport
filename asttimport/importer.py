...
class ExcelImporter:
    def __init__(self, path: Path):
        ...
        self.assignments, self.subjects, self.groups = self._import_assignments(
            workbook.get_sheet_by_name("Beosztas")
        )
    ...
    def _import_classrooms(self, worksheet: CalamineSheet) -> Iterable[Classroom]:
        ...
        for row in data:
            yield Classroom(
                name=row["Terem"],
                type=CLASSROMTYPE_MAPPING.get(row["Terem tipus"], ClassroomType.NORMAL),
    ...
    def _import_assignments(
    ...
    ) -> tuple[list[Assignment], list[Subject], list[Group]]:
        data = self._convert_to_named_list(worksheet)
        ...
        for row in data:
            grade = row["Évfolyam"]
            class_ = (
                self.classes[row["Osztály"]]
                if row["Osztály"] not in ("", "Teljes")
                else None
            )
            group_name = (
            ...
            if group_name:
                if class_ is not None:
                    groups = [
                        Group(name=group_name, class_=class_),
                    ]
                else:
                    groups = [
