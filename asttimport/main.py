...
def main():
    ...
    with Path("orarend.csv").open("w+") as f:
        ...
        c.writerow(["Tanár", "Tantárgy", "Évfolyam", "Osztály", "Óraszám", "Időszak", "AN óra", "Csoport"])

        for assignment in importer.assignments:
            class_names = ",".join([c.name for c in assignment.classes])
            grades = {
                str(c.grade) for c in assignment.classes
            } | {
    ...
    for assignment in importer.assignments:
        if assignment.key in assignments:
            print("DUPLICATE:", assignment.key)#, assignment, assignments[assignment.key])
        else:
